...
function search_queue() {
    ...
    var map = graph();
    var queue = ['sam', 'alice', 'claire', 'bob', 'peggy', 'thom', 'anuj', 'jonnya'];
    var checked = new Set();
    while (queue.length > 0) {
        var element_1 = queue.shift();
        console.log('checked', checked);
        if (checked.has(element_1) === false) {
            if (checkSeller(element_1) === true) {
                return element_1;
            }
            else {
                checked.add(element_1);
                queue.push.apply(queue, map.get(element_1));
            }
        }
    }
    return null;
