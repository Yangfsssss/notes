...
import { Doc, ProducerIF, ProvinceIF } from './type';
...
export class Producer {
	_province: ProvinceIF;
	_cost: number;
	_name: string;
	_production: number;

	constructor(aProvince: ProvinceIF, data: ProducerIF) {
	...
	set production(amountStr) {
		const amount = typeof amountStr === 'string' ? parseInt(amountStr) : amountStr;
...
const producer = new Producer(province, { name: 'Sinope', cost: 10, production: 6 });
