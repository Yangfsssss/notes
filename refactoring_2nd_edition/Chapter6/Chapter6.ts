import * as all from '../../react-update/dom-store';
/** Chapter6：第一组重构 */
...
function test() {
  ...
  function baseRate(m: number, y: number) {
    return 0.3;
  }
  ...
  const taxableCharge2 = taxableChargeFn(aReading2);
  ...
  //-4
  // const taxableCharge3 = taxableChargeFn(aReading3);
  //-5
  ...
  // const basicChargeAmount2 = calculateBaseCharge(aReading);
  //-2 -3
  const basicChargeAmount2 = aReading.baseCharge;

  //然后我用搬移函数（198）把 calculateBaseCharge 搬到新类中。-2

  //搬移的同时，我会顺便运用函数改名（124），按照我喜欢的风格对这个函数改名。 -3

  //用这个名字，Reading 类的客户端将不知道 baseCharge 究竟是一个字段还是推演计算出的值。
  //这是好事，它符合“统一访问原则”（Uniform Access Principle）[mf-ua]。

  //现在我可以修改客户端 1 的代码，令其调用新的方法，不要重复计算基础费用。 -4

  //运用提炼函数（106）将计算应税费用（taxable charge）的逻辑提炼成函数
  //然后我运用搬移函数（198）将其移入 Reading 类 -5

  //由于所有派生数据都是在使用时计算得出的，所以对存储下来的读数进行修改也没问题。
  //一般而论，我更倾向于使用不可变的数据；
  //但很多时候我们必须得使用可变数据（比如 JavaScript 整个语言生态在设计时就没有考虑数据的不可变性）。
